...
TOKEN = "7062229353:AAHkk3s_hFBYiLLCqGqIfbt9vDXVOA0HIXI"
MONGO_URI = 'mongodb+srv://Bishal:Bishal@bishal.dffybpx.mongodb.net/?retryWrites=true&w=majority&appName=Bishal'
FORWARD_CHANNEL_ID = -4798670710
CHANNEL_ID = -4798670710
error_channel_id = -4798670710
...
# Define admin ID(s)
ADMIN_IDS = 1908670857# List of admin user IDs (replace with actual IDs)

CHANNEL_ID = '4798670710'  # The chat ID for your channel or group

# Check if the user is an admin
def is_user_admin(user_id, chat_id):
    # First check if the user is in the admin list
    if user_id in ADMIN_IDS:
        return True
    
    try:
        # If not, check the user's status in the chat (e.g., 'administrator', 'creator')
        return bot.get_chat_member(chat_id, user_id).status in ['administrator', 'creator']
    except:
        return False

# Command handler for approve, disapprove, and add commands
@bot.message_handler(commands=['approve', 'disapprove', 'add'])
def approve_or_disapprove_user(message):
    ...
    # Handle admin check and command validation
    if not is_admin:
    ...
    if len(cmd_parts) < 2:
        bot.send_message(chat_id, "*⚠️ Hold on! Invalid command format.*\n"
                                   ...
                                   "*3. /add <user_id>*", parse_mode='Markdown')
        return
    ...
    if command == '/approve':
        if len(cmd_parts) != 4:
            bot.send_message(chat_id, "*⚠️ Hold on! Invalid command format for approve.*\n"
                                       "*Please use the following format:*\n"
                                       "*1. /approve <user_id> <plan> <days>*", parse_mode='Markdown')
            return
        approve_user_id = cmd_parts[1]
        plan = cmd_parts[2]
        days = cmd_parts[3]
        # Implement logic for approving the user (e.g., add to database or approve their plan)
        bot.send_message(chat_id, f"User {approve_user_id} has been approved for {plan} plan for {days} days.", parse_mode='Markdown')

    # Handle /disapprove command
    elif command == '/disapprove':
        disapprove_user_id = cmd_parts[1]
        # Implement logic for disapproving the user (e.g., remove them from an approved list)
        bot.send_message(chat_id, f"User {disapprove_user_id} has been disapproved.", parse_mode='Markdown')

    # Handle /add command
    elif command == '/add':
        if len(cmd_parts) != 2:
            bot.send_message(chat_id, "*⚠️ Hold on! Invalid command format for add.*\n"
                                       "*Please use the following format:*\n"
                                       "*1. /add <user_id>*", parse_mode='Markdown')
            return
        new_user_id = cmd_parts[1]
        # Add the user ID to the approved users list (this is a simple example)
        if new_user_id not in approved_users:
            approved_users.append(new_user_id)
            bot.send_message(chat_id, f"User {new_user_id} has been added to the approved list.", parse_mode='Markdown')
        else:
            bot.send_message(chat_id, f"User {new_user_id} is already in the approved list.", parse_mode='Markdown')
